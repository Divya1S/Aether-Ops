...
For a **fully live end-to-end run** — a real commit + diff read from the live
GitHub API, a real p99 series from a live Prometheus, and a hypothesis
generated by a real local model, closed with a per-item provenance report —
`make live-demo` (`python3 -m aetherops.live_demo`). On unscripted live data
the grounded answer is often *"unclassified"* — the model correctly declining
to invent a cause is the guardrail working, not a failure. A captured run
(`qwen2.5:7b` + the public Prometheus demo + this repo, network-live):

```
AetherOps — END-TO-END LIVE run (live GitHub · live Prometheus · live model)

[1] LIVE GitHub  (Divya1S/Aether-Ops)
    change   : 2c35fb9a88  "Add end-to-end LIVE demo: live GitHub + live Prometheus…"
    diff     : 9777 chars of real unified diff
    cite     : github://commit/2c35fb9a88a3b894b154d55714cdeb8771a59245
[2] LIVE Prometheus  (https://prometheus.demo.prometheus.io)
    query    : histogram_quantile(0.99, sum by (le) (rate(prometheus_http_request…)))
    p99 (ms) : [99.0, 99.0, 99.0, 99.0]  (4 live samples)
    cite     : prometheus://query/query_metrics
[3] LIVE model  (Ollama · qwen2.5:7b)  — reasoning over the live evidence…
    hypothesis (generated live):
      Insufficient evidence.
      Recommended class: unclassified

Provenance — every input was live, the model was live
  evidence E1  <- LIVE Prometheus   prometheus://query/query_metrics
  evidence E2  <- LIVE GitHub       github://commit/2c35fb9a88a3b894b154d55714cdeb8771a59245
  evidence E3  <- LIVE GitHub       diff of 2c35fb9a88
  model call   <- backend=ollama  served=ollama/qwen2.5:7b
                  tokens=571  latency=2684.7ms  est_prod_cost=$0.003055
```

Real commit, real live metrics, real local model — and it declined to invent a
