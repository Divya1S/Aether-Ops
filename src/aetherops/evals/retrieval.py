...
The GATED track is always TF-IDF: like golden-scenario replay, the release
gate must be deterministic and runnable in CI with no network. An opt-in
SEMANTIC track (free local Ollama embeddings, `run_semantic_retrieval_eval`)
is REPORTED but never gated — it lifts the paraphrase subset that lexical
retrieval cannot reach, and skips cleanly when Ollama isn't available.
"""
from __future__ import annotations

import random

from aetherops.rag.evalset import LABELED_QUERIES, PARAPHRASE_COUNT
...
def evaluate_strategy(chunker: str, embedder: str = "tfidf") -> dict:
    store = RagStore(chunker=chunker, embedder=embedder)
    p1_hits: list[int] = []             # per-query precision@1 indicator
    para_hits: list[int] = []           # ...over the paraphrase subset only
    p5 = r5 = mrr = 0.0
    ...
    for i, (query, relevant) in enumerate(LABELED_QUERIES):
        ...
        hit1 = 1 if ranked and ranked[0] in relevant_set else 0
        p1_hits.append(hit1)
        if i >= n - PARAPHRASE_COUNT:
            para_hits.append(hit1)
        p5 += len(hits) / 5.0
    ...
    return {
        ...
        "paraphrase_precision_at_1": round(sum(para_hits) / len(para_hits), 3),
...
def run_semantic_retrieval_eval() -> dict:
    """Opt-in SEMANTIC track: free local dense embeddings (Ollama), REPORTED
    and never gated — the same discipline as the live-model golden track. It
    lifts the paraphrase subset a lexical retriever can't reach. Returns
    {'available': False} when Ollama or the embed model isn't up, so CI
    (which has neither) simply skips it instead of failing."""
    try:
        strategies = {name: evaluate_strategy(name, embedder="ollama")
                      for name in STRATEGIES}
    except Exception as exc:            # network / backend / model unavailable
        return {"available": False, "reason": str(exc)[:140]}
    return {"available": True, "strategies": strategies}
